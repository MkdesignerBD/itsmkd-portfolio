...
export default function WorkEthicScrollSection({ children }) {
  ...
  return (
    ...
    >
      ...
      <div className="sticky top-6 md:top-8 z-10 max-w-[1360px] w-full mx-auto px-6 md:px-12 mb-6 md:mb-8 pointer-events-none">
        <p className="text-black/55 text-[15px] md:text-[17px] font-normal mb-1.5">
          Work Ethic
        </p>

        <h2 className="text-[32px] sm:text-[42px] md:text-[50px] leading-[1.08] tracking-tight">
      ...
      {slides.map((s, i) => (
        <div
          key={i}
          className="sticky top-[175px] md:top-[205px] max-w-[1360px] w-full mx-auto px-6 md:px-12 mb-10 md:mb-14 pointer-events-auto"
          style={{
            zIndex: 15 + i * 5,
          }}
        >
          <div className="w-full bg-white rounded-[36px] md:rounded-[44px] shadow-[0_20px_60px_rgba(0,0,0,0.08)] border border-black/[0.04] p-6 sm:p-8 md:p-12 lg:p-14 transition-all duration-300">
            <div className="grid grid-cols-1 lg:grid-cols-[500px_1fr] gap-8 md:gap-12 lg:gap-16 items-center">
              {/* Left: Product Image */}
              <div className="w-full flex justify-center">
                <div className="w-full max-w-[480px] aspect-square rounded-[24px] md:rounded-[28px] overflow-hidden bg-black/5 shadow-[0_12px_32px_rgba(0,0,0,0.06)]">
                  <img
                    src={s.img}
                    alt={s.title}
              ...
              <div className="flex items-center justify-between gap-6 lg:gap-12">
                <div className="flex-1 max-w-[560px]">
                  {/* Kicker */}
                  <p className="text-[13px] md:text-[14px] font-medium tracking-[0.18em] uppercase text-black/40">
                    {s.kicker}
                  </p>

                  {/* Title */}
                  <h3 className="mt-3 text-[32px] sm:text-[38px] md:text-[44px] font-semibold text-black tracking-tight leading-[1.1]">
                    {s.title}
                  </h3>

                  {/* Thin Horizontal Divider */}
                  <div className="mt-5 mb-7 h-[1px] bg-black/[0.08] w-full" />

                  {/* Description Paragraph */}
                  <p className="text-[16px] md:text-[17px] leading-[1.68] text-black/75 font-normal">
                    {s.text}
                  </p>
                  ...
                  <div className="mt-8 md:mt-10">
                    <a
                      href={s.link}
                      className="inline-flex items-center justify-center px-9 py-3 rounded-[12px] border border-black/25 bg-white text-black/85 text-[15px] font-medium hover:bg-black hover:text-white hover:border-black transition-all duration-200"
                    >
                      Case Studies
                    </a>
                  </div>
